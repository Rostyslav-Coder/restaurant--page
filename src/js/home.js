// HOME PAGE TO RESTAURANT PAGE

const HOME_PAGE = `
  <div class="home-page"> 
    <div class="back-bg">
      <img src="images/universe.png" alt="earth" />
    </div>
    <div class="text parallax">
      <p>There is only one <span>real Pizzeria</span> on the Moon</p>
    </div>
    <div class="earth parallax">
      <img src="images/earth.png" alt="earth" />
    </div>
    <div class="front-bg">
      <img src="images/moon-landscape.png" alt="moon landscape" />
    </div>
    <div class="moon-shatle">
      <img src="images/moon-station.png" alt="moon shatle" />
      <p>I only use the freshest ingredients.</p>
      <p>Just don’t ask where I get them from.</p>
    </div>
    <div class="pizza-man-1 astronaut">
      <img src="images/pizza-man-1.png" alt="pizza man" />
      <p>We make the most delicious pizza on the entire moon.</p>
      <p>This is the only pizzeria on the moon.</p>
    </div>
    <div class="pizza-man-2 astronaut">
      <img src="images/pizza-man-3.png" alt="pizza man" />
      <p>We can deliver pizza to you anywhere in the world.</p>
      <p>We'll drop it right on your doorstep when we're passing by.</p>
    </div>
  </div>
`;

export { HOME_PAGE };
